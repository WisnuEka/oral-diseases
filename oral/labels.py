...
class Labels:
    def __init__(self, base_data_path: str = None, choosen_labels: list = ('Caries', 'Ulcer', 'Gingivitis')):
        if base_data_path is None:
            self.base_data_path = os.path.join('data',
                                               'Caries_Gingivitus_ToothDiscoloration_Ulcer-yolo_annotated-Dataset',
                                               'Caries_Gingivitus_ToothDiscoloration_Ulcer-yolo_annotated-Dataset',
                                               'Data')
        self.choosen_labels = choosen_labels
                ...
                self.tagged_images_with_regions.append(
                    ImageFileCreateEntry(
                        ...
                        regions=[
                            ...
                            for region in annotation.boxes if self.id2label[int(region.label)] in self.choosen_labels
                        ]
                    )
                )
